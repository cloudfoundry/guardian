...
var _ = Describe("Idfinder", func() {
	Context("FindID", func() {
		...
		Context("when a path is provided", func() {
			It("returns the ID", func() {
				id, err := idfinder.FindID("/hello/store/path", "/hello/store/path/1200/bundles/1234-my-id")
				Expect(err).NotTo(HaveOccurred())
				Expect(id).To(Equal("1234-my-id"))
			})
	...
	Context("SubStorePath", func() {
		It("returns the correct sub store path", func() {
			storePath, err := idfinder.FindSubStorePath("/hello/store/path", "/hello/store/path/1200/bundles/1234-my-id")
			Expect(err).NotTo(HaveOccurred())
		...
		Context("when the path is not valid", func() {
			It("returns an error", func() {
				_, err := idfinder.FindSubStorePath("/hello/store/path", "/hello/store/path/bundles/1234-my-id")
				Expect(err).To(MatchError(ContainSubstring("unable to match substore in path `/hello/store/path/bundles/1234-my-id`")))
			})
		})
	})
})
