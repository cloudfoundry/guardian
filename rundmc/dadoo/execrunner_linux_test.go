			...
			Describe("ExitStatus", func() {
				Context("when the process does not exit immediately", func() {
					...
					It("does not return until the exit pipe is closed", func() {
						process, err := runner.Run(log, processID,
							&runrunc.PreparedSpec{Process: specs.Process{Args: []string{"Banana", "rama"}}},
							bundlePath, processPath, "some-handle", nil, garden.ProcessIO{})
						Expect(err).NotTo(HaveOccurred())
				...
				It("returns the exit code of the dadoo process", func() {
					...
					exitStatusChan := process.ExitStatus()
					status := garden.ProcessStatus{}
					Eventually(exitStatusChan, "10s", "2s").Should(Receive(&status))
					Expect(status.Err).NotTo(HaveOccurred())
					Expect(status.Code).To(Equal(42))
				})
				...
				Context("when the exitfile is empty", func() {
					It("returns an error", func() {
						...
						status := garden.ProcessStatus{}
						Eventually(exitStatusChan, "10s", "2s").Should(Receive(&status))
						Expect(status.Err).To(MatchError(ContainSubstring("the exitcode file is empty")))
					})
				})
				...
				Context("when the exitfile does not exist", func() {
					It("returns an error", func() {
						...
						status := garden.ProcessStatus{}
						Eventually(exitStatusChan, "10s", "2s").Should(Receive(&status))
						Expect(status.Err).To(MatchError(ContainSubstring("could not find the exitcode file for the process")))
					})
				})
				...
				Context("when the exitcode file doesn't contain an exit code", func() {
					It("returns an error", func() {
						...
						status := garden.ProcessStatus{}
						Eventually(exitStatusChan, "10s", "2s").Should(Receive(&status))
						Expect(status.Err).To(MatchError(ContainSubstring("failed to parse exit code")))
					})
				})
			})
