...
var _ = Describe("CPU Limits", func() {
	...
	JustBeforeEach(func() {
		...
		currentCgroup, err := exec.Command("sh", "-c", "cat /proc/self/cgroup | grep 'cpu:' | awk -F ':' '{print $3}'").CombinedOutput()
		Expect(err).NotTo(HaveOccurred())
		cgroupName := strings.TrimSpace(string(currentCgroup))
		containerCpuCgroupPath = fmt.Sprintf("/tmp/test-garden-%d/cgroups-%d/cpu/%s/%s", GinkgoParallelNode(), GinkgoParallelNode(), cgroupName, container.Handle())
	})
	...
	Context("when started with low cpu limit turned on", func() {
		...
		Context("when when a container with cpu limits is created", func() {
			It("throttles process cpu usage", func() {
				periods, throttled, time, err := parseCpuStats(filepath.Join(containerCpuCgroupPath, "cpu.stat"))
				Expect(err).NotTo(HaveOccurred())
			...
			It("sets cpu.cfs_period_us to 100000 (100ms)", func() {
				period, err := ioutil.ReadFile(filepath.Join(containerCpuCgroupPath, "cpu.cfs_period_us"))
				Expect(err).NotTo(HaveOccurred())
